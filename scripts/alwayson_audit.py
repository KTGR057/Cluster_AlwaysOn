...
def html_report(report):
    ...
    for cluster in report["clusters"]:
        ...
        for vm in cluster["nodes"]:
            ...
            node_cards.append("<article class='node-card'><div class='node-heading'><div><span class='eyebrow'>Nodo</span><h3>%s</h3></div><span class='site'>%s</span></div><div class='node-summary'><div class='summary-section'><strong>CPU</strong><div><b>Host ESXi</b><span>%s</span></div><div><b>vCPU / Shares</b><span>%s vCPU · %s</span></div><div><b>Topologia actual</b><span>%s</span></div></div><div class='summary-section'><strong>Memoria</strong><div><b>RAM asignada</b><span>%s GB</span></div><div><b>Reserva</b><span>%s MB (%s%%)</span></div><div><b>Estado</b><span>%s</span></div></div><div class='summary-section'><strong>Almacenamiento</strong><div><b>Controladoras</b><span>%s</span></div><div><b>Aprovisionamiento</b><span>%s</span></div></div><div class='summary-section'><strong>Red</strong><div><b>Adaptador / VLAN</b><span>%s</span></div><div><b>MTU</b><span>%s</span></div></div><div class='summary-section'><strong>OS</strong><div><b>Estado</b><span>%s</span></div><div><b>Configurado</b><span>%s</span></div><div><b>VMware Tools reporta</b><span>%s</span></div></div><div class='summary-section'><strong>Plataforma</strong><div><b>Hardware virtual</b><span>%s</span></div><div><b>VMware Tools version</b><span>%s</span></div></div></div><div class='cpu-advice %s' style='%s'><b>Recomendacion de topologia CPU</b><span>%s</span><small>%s</small></div><div class='os-advice'><b>Recomendacion de OS</b><span>%s</span><small>%s</small></div><div class='node-footer'><span>Discos: %s</span><span>Red: %s</span></div></article>" % tuple(html.escape(str(x)) for x in [vm["name"], vm.get("vcenter", "offline"), vm["host"], vm["cpu"]["vcpus"], shares_text, recommendation["current"], round(vm["memory"]["assigned_mb"] / 1024, 1), vm["memory"]["reservation_mb"], vm["memory"]["reservation_percent"], vm["memory"]["reservation_status"], ", ".join(c["type"] for c in vm["controllers"]) or "No informado", disks or "No informado", networks or "No informado", ", ".join(str(n["mtu"]) for n in vm["networks"]) or "No informado", vm["os"]["status"], vm["os"]["configured"], vm["os"]["tools_reported"], vm["platform"]["virtual_hardware"], vm["platform"]["tools_version"], recommendation["status"].lower(), advice_style, recommendation["message"], recommendation["host_baseline"], vm["os"]["recommendation"], "Configurado: %s · VMware Tools: %s" % (vm["os"]["configured"], vm["os"]["tools_reported"]), disks or "No informado", networks or "No informado"]))
